...
% of two Underlying Assets which ARE CORRELATED. See the other file for
% the case without correlation. We use a PDE method here with discretisation.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
close all; clear;
%
%% Parameters :
...
r = 0.1 / 100; sigma1 = 0.6; sigma2 = 0.4; rho = 0.4;
...
M = spalloc(num_points, num_points, num_points * 9);
for i = 1:num_points
    if min(abs(i - Boundary)) == 0 % i is a boundary index
        ...
        M(i, i - 1) = M(i, i - 1) + (sigma2^2 / 2) / dx2^2;
        % mixed second order derivative for correlation
        coeff = 0.5 * rho * sigma1 * sigma2;
        M(i, i + 1 + (N2 + 1)) = coeff / (4 * dx1 * dx2);
        M(i, i + 1 - (N2 + 1)) = -coeff / (4 * dx1 * dx2);
        M(i, i - 1 + (N2 + 1)) = -coeff / (4 * dx1 * dx2);
        M(i, i - 1 - (N2 + 1)) = coeff / (4 * dx1 * dx2);
    end
